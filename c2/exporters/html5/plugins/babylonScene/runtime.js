...
cr.plugins_.babylonSceneSLE = function (runtime) {
...
(function () {
	...
	// *** CHANGE THE PLUGIN ID HERE *** - must match the "id" property in edittime.js
	//                            vvvvvvvv
	var pluginProto = cr.plugins_.babylonSceneSLE.prototype;
	...
	instanceProto.onCreate = function () {
		...
		if (document.getElementById('babylonCanvas' + this.uid) === null) {
			...
			if (this.properties[20] == 0) // Render inside C2
			{
			...
			} else if (this.properties[20] == 1) // Render under C2
			{
				this.canvas.style.zIndex = "0";
			...
			this.engine = new BABYLON.Engine(this.canvas, this.properties[1]);
			this.engine.enableOfflineSupport = false;
			this.engine.setDepthBuffer(this.properties[2]);
			this.engine.renderEvenInBackground = this.properties[3];
			this.engine.cullBackFaces = this.properties[4];
			...
			var ins = this;
			BABYLON.SceneLoader.Load("", ins.properties[0], ins.engine, function (newScene) {
				ins.scene = newScene;
				ins.scene.MinDeltaTime = ins.properties[6];
				ins.scene.MaxDeltaTime = ins.properties[7];
				ins.scene.autoClear = ins.properties[8];
				var clearCol = getRGB(ins.properties[9]);
				var ambCol = getRGB(ins.properties[10]);
				ins.scene.clearColor = BABYLON.Color3.FromInts(clearCol.r, clearCol.g, clearCol.b);
				ins.scene.ambientColor = BABYLON.Color3.FromInts(ambCol.r, ambCol.g, ambCol.b);
				ins.scene.forceWireframe = ins.properties[11];
				ins.scene.forcePointsCloud = ins.properties[12];
				ins.scene.forceShowBoundingBoxes = ins.properties[13];
				ins.scene.shadowsEnabled = ins.properties[14];
				ins.scene.lightsEnabled = ins.properties[15];
				ins.scene.texturesEnabled = ins.properties[16];

				// Debug
				if (ins.properties[17] == 1) {
					ins.scene.debugLayer.show();
...
document.getElementById("inspector-host").style.zIndex = "5";
				...
				ins.scene.collisionsEnabled = true;
				var grav = ins.properties[18].split(",");
				grav[0] = parseFloat(grav[0]);
				grav[1] = parseFloat(grav[1]);
				grav[2] = parseFloat(grav[2]);
				// Wait for textures and shaders to be ready
				ins.scene.executeWhenReady(function () {

					ins.engine.resize();
					ins.runtime.trigger(cr.plugins_.babylonSceneSLE.prototype.cnds.IsSceneReady, ins);
					// Once the scene is loaded, just register a render loop to render it
					ins.engine.runRenderLoop(function () {
						ins.scene.render();
					});

				});
			}, function (progress) {
				ins.progress = progress.loaded;
				ins.total = progress.total;
				// To do: give progress feedback to user
				if (progress.lengthComputable) {
					ins.engine.loadingUIText = "Loading, please wait..." + (progress.loaded * 100 / progress.total).toFixed() + "%";
				} else {
					var dlCount = progress.loaded / (1024 * 1024);
					ins.engine.loadingUIText = "Loading, please wait..." + Math.floor(dlCount * 100.0) / 100.0 + " MB already loaded.";
				}
			});
		}
	...
	instanceProto.drawGL = function (glw) {
		if (this.properties[20] == 0) // Render inside C2
		{
			this.canvas = document.getElementById('babylonCanvas' + this.uid)
	...
	Cnds.prototype.IsSceneReady = function () {
		this.runtime.scenes[this.properties[19]] = this.scene;
		this.runtime.scenes[this.properties[19]].canvas = this.canvas;
	...
	Acts.prototype.SceneLoad = function (file) {
		var ins = this;
		var eng = this.engine;
		BABYLON.SceneLoader.Load("", file, ins.engine, function (newScene) {
			ins.scene = newScene;
			ins.scene.MinDeltaTime = ins.properties[6];
			ins.scene.MaxDeltaTime = ins.properties[7];
			ins.scene.autoClear = ins.properties[8];
...
document.getElementById("inspector-host").style.zIndex = "5";
	...
	Acts.prototype.DisposePhysics = function () {
...
document.getElementById("inspector-host").style.zIndex = "5";
	...
	Exps.prototype.SceneLoadingProgress = function (ret) {
		ret.set_float(parseFloat(this.progress));
	};
	Exps.prototype.SceneLoadingTotal = function (ret) {
		ret.set_float(parseFloat(this.total));
	};
