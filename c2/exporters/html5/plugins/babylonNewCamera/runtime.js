...
cr.plugins_.babylonNewCamera = function (runtime) {
...
(function () {
	...
	// *** CHANGE THE PLUGIN ID HERE *** - must match the "id" property in edittime.js
	//                            vvvvvvvv
	var pluginProto = cr.plugins_.babylonNewCamera.prototype;
	...
	instanceProto.tick2 = function () {
		...
		if (this.runtime.scenes[this.properties[1]] && !this.created && !this.runtime.scenes[this.properties[1]].getCameraByName(this.properties[0])) { // Scene is ready
			var scene = this.runtime.scenes[this.properties[1]];
			var camname = this.properties[0];
			var xpos = this.x - (this.runtime.original_width / 2);
			var ypos =  - (this.y - (this.runtime.original_height / 2));
			var zpos = this.properties[3];
			var target = getXYZ(this.properties[4], this);
			var checkColl = this.properties[5];
			var applyGrav = this.properties[6];
			var mode = this.properties[7];
			var fov = this.properties[8];
			var angs = this.properties[9];
			var alpha = this.properties[10];
			var beta = this.properties[11];
			var radius = this.properties[12];
			var zoomf = this.properties[13];
			var targm = this.properties[14];
			var dist = this.properties[15];
			var hoff = this.properties[16];
			var roff = this.properties[17];
			var camacc = this.properties[18];
			var maxcams = this.properties[19];
			if (this.properties[2] == 0) // Target camera
			{
				var camera = new BABYLON.TargetCamera(camname, new BABYLON.Vector3(xpos, ypos, zpos), scene);
				camera.setTarget(new BABYLON.Vector3(target[0], target[1], target[2]));
				scene.activeCamera = camera;
			} else if (this.properties[2] == 1) // Free camera
			{
				var camera = new BABYLON.FreeCamera(camname, new BABYLON.Vector3(xpos, ypos, zpos), scene);
				camera.setTarget(new BABYLON.Vector3(target[0], target[1], target[2]));
				camera.angularSensibility = angs;
				scene.activeCamera = camera;
			} else if (this.properties[2] == 2) // ArcRotateCamera
			{
				var camera = new BABYLON.ArcRotateCamera(camname, alpha, beta, radius, new BABYLON.Vector3(0,0,0), scene);
				
				camera.zoomOnFactor = zoomf;
				scene.activeCamera = camera;
			} else if (this.properties[2] == 3) // FollowCamera
			{
				var camera = new BABYLON.FollowCamera(camname, new BABYLON.Vector3(xpos, ypos, zpos), scene);
				camera.lockedTarget = scene.getMeshByName(targm); // target any mesh or object with a "position" Vector3
				camera.radius = radius; // how far from the object to follow
				camera.heightOffset = hoff; // how high above the object to place the camera
				camera.rotationOffset = roff; // the viewing angle
				camera.cameraAcceleration = camacc; // how fast to move
				camera.maxCameraSpeed = maxcams; // speed limit
				scene.activeCamera = camera;
			}
			camera.position = (new BABYLON.Vector3(xpos, ypos, zpos));
			
			camera.checkCollisions = checkColl;
			camera.applyGravity = applyGrav;
			camera.mode = mode;
			camera.fov = fov;
			this.created = true;
	...
	Cnds.prototype.CamOnCollide = function (m) {
		if (this.runtime.scenes[this.properties[1]]) {
			...
			var mesh = this.runtime.scenes[this.properties[1]].getMeshByName("mesh"+m.getFirstPicked().uid);
	...
	Acts.prototype.CameraSetTarg = function (m) {
		var camera = this.runtime.scenes[this.properties[1]].getCameraByName(this.properties[0]);
		var target = this.runtime.scenes[this.properties[1]].getMeshByName("mesh"+m.getFirstPicked().uid);
