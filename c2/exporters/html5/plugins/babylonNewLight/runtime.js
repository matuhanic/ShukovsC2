...
cr.plugins_.babylonNewLight = function (runtime) {
...
(function () {
	...
	// *** CHANGE THE PLUGIN ID HERE *** - must match the "id" property in edittime.js
	//                            vvvvvvvv
	var pluginProto = cr.plugins_.babylonNewLight.prototype;
	...
	instanceProto.tick2 = function () {
		...
		if (this.runtime.scenes[this.properties[1]] && !this.created && !this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid)) {
			var t = this.properties[5].split(",");
			var xpos = this.x - (this.runtime.original_width / 2);
			var ypos = - (this.y - (this.runtime.original_height / 2));
			var zpos = this.properties[4];
			var pos = new BABYLON.Vector3(xpos, ypos, zpos);
			var targ = new BABYLON.Vector3(parseFloat(t[0]), parseFloat(t[1]), parseFloat(t[2]));
			switch (this.properties[3]) {
				case 0:
					var light = new BABYLON.DirectionalLight("light" + this.uid, new BABYLON.Vector3(0, 0, 0), this.runtime.scenes[this.properties[1]]);
					light.position = pos;
					light.setDirectionToTarget(new BABYLON.Vector3(light.position.x+targ.x,light.position.y+targ.y,light.position.z+targ.z));
					break; //Dir
				case 1:
					var light = new BABYLON.PointLight("light" + this.uid, pos, this.runtime.scenes[this.properties[1]]);
					light.position = pos;
					break; //Point
				case 2:
					var light = new BABYLON.SpotLight("light" + this.uid, pos, new BABYLON.Vector3(0, 0, 0), this.properties[12], this.properties[13], this.runtime.scenes[this.properties[1]]);
					light.position = pos;
					light.setDirectionToTarget(new BABYLON.Vector3(light.position.x+targ.x,light.position.y+targ.y,light.position.z+targ.z));
					//light.rotation.z = this.angle;
					break; //Spot
				case 3:
					var light = new BABYLON.HemisphericLight("light" + this.uid, new BABYLON.Vector3(0, 0, 0), this.runtime.scenes[this.properties[1]]);
					light.position = pos;
					light.setDirectionToTarget(new BABYLON.Vector3(light.position.x+targ.x,light.position.y+targ.y,light.position.z+targ.z));
					break; //Hemi
			}
			...
			light.diffuse.r = getRGB(this.properties[9]).r;
			light.diffuse.g = getRGB(this.properties[9]).g;
			light.diffuse.b = getRGB(this.properties[9]).b;
			light.specular.r = getRGB(this.properties[10]).r;
			light.specular.g = getRGB(this.properties[10]).g;
			light.specular.b = getRGB(this.properties[10]).b;
			light.radius = this.properties[6];
			light.range = this.properties[7];
			light.intensity = this.properties[8] / 100;

			if (this.properties[2] == 3) {
				light.groundColor.r = getRGB(this.properties[11]).r;
				light.groundColor.g = getRGB(this.properties[12]).g;
				light.groundColor.b = getRGB(this.properties[13]).b;
			};
			this.runtime.trigger(cr.plugins_.babylonNewLight.prototype.cnds.IsLightReady, this);
			this.created = true;
		}

		if (this.properties[2] == 1 && this.runtime.scenes[this.properties[1]] && this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid)) // Construct 2 is taking the wheel
		{
			...
			if (!this.done) {
				...
				//var xscale = this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).scaling.x;
				//var yscale = this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).scaling.y;
				//this.width = (this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).getBoundingInfo().boundingBox.extendSize.x * xscale) * 2;
				//this.height = (this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).getBoundingInfo().boundingBox.extendSize.y * yscale) * 2;
				this.x = this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).position.x;
				this.y = -this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).position.y;
				if(this.properties[3] != 1)
				{
					this.angle = this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).direction.x;
				}
				...
				if(this.properties[3] != 1)
				{
				this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).direction.x = this.angle; // Euler
				}


				this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).position.x = this.x;
				this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).position.y = -this.y;
				//this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).scaling.x = this.width / (this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).getBoundingInfo().boundingBox.extendSize.x * 2);
				//this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).scaling.y = this.height / (this.runtime.scenes[this.properties[1]].getLightByName("light"+this.uid).getBoundingInfo().boundingBox.extendSize.y * 2);

				this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).setEnabled(this.visible);
				this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid).intensity = this.opacity/100;
	...
	Acts.prototype.LightSetDiff = function (r, g, b) {
		var light = this.runtime.scenes[this.properties[1]].getLightByName("light" + this.uid);
		light.diffuse.r = r;
		light.diffuse.g = g;
		light.diffuse.b = b;
	};
	...
	Acts.prototype.LightSetParent = function (ptype, p) {
		...
		if (ptype == 0) {
			...
			var parnt = this.runtime.scenes[this.properties[1]].getMeshByName("camera" + p.getFirstPicked().uid);
		}
