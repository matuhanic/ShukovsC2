...
/////////////////////////////////////
// Behavior class
cr.behaviors.babylonEDir = function (runtime) {
	this.runtime = runtime;
};

(function () {
	var behaviorProto = cr.behaviors.babylonEDir.prototype;
	...
	behinstProto.tick = function () {
		if (this.inst.mytype == "_newmesh") {
			if (this.runtime.scenes[this.inst.properties[1]] && this.runtime.scenes[this.inst.properties[1]].getMeshByName("mesh" + this.inst.uid)) {
				...
				var mesh = scene.getMeshByName("mesh" + this.inst.uid);
				...
				if (this.dx !== 0 || this.dy !== 0) {
					...
					var oldx = mesh.position.z;
					var oldy = mesh.position.x;
					var oldangle = this.inst.angle;

					// Attempt X movement

					mesh.moveWithCollisions(new BABYLON.Vector3(0, 0, this.dx * dt));
					this.inst.set_bbox_changed();

					collobj = this.runtime.testOverlapSolid(this.inst);
					if (collobj) {
						mesh.moveWithCollisions(new BABYLON.Vector3(0, 0, oldx));
						this.dx = 0;
						this.inst.set_bbox_changed();
						this.runtime.registerCollision(this.inst, collobj);
					}

					mesh.moveWithCollisions(new BABYLON.Vector3(this.dy * dt, 0, 0));
					this.inst.set_bbox_changed();

					collobj = this.runtime.testOverlapSolid(this.inst);
					if (collobj) {
						mesh.moveWithCollisions(new BABYLON.Vector3(oldy, 0, 0));
						this.dy = 0;
						this.inst.set_bbox_changed();
						this.runtime.registerCollision(this.inst, collobj);
					}
		...
		else if(this.inst.mytype == "_mesh")
		{
			if (this.runtime.scenes[this.inst.properties[1]] && this.runtime.scenes[this.inst.properties[1]].getMeshByName(this.inst.properties[0])) {
				...
				var scene = this.runtime.scenes[this.inst.properties[1]];
				var mesh = scene.getMeshByName(this.inst.properties[0]);
				mesh.checkCollisions = true;
				var dt = this.runtime.getDt(this.inst);
	...
	behinstProto.getDebuggerValues = function (propsections) {
		propsections.push({
			...
			"properties": [{
				...
				"name": "Overall speed",
				"value": Math.sqrt(this.dx * this.dx + this.dy * this.dy)
			}, {
				"name": "Angle of motion",
				"value": cr.to_degrees(Math.atan2(this.dy, this.dx)),
				"readonly": true
			}, {
				"name": "Max speed",
				"value": this.maxspeed
