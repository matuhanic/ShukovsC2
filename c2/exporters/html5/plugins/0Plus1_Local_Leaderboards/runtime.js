...
(function ()
{
	...
	function Utils(){
		...
		//Sort from highest to lowest, using the Natural sort method
	...
	function Acts() {};
	//AUTH
	Acts.prototype.reportScore = function (id,score,name)
	{
		utils.init(id); //checks and creates object based on the local storage existing
			utils.local_storage_masterArray[id].push(score + " " + name); //Push score + name to leaderboard array
			utils.sortArray(id);
			utils.cleanArray(id);
	...
	//Currentname doesn´t work
	Exps.prototype.currentName = function (ret)	
	{
		ret.set_string( currentLeaderboard_score );
	};
