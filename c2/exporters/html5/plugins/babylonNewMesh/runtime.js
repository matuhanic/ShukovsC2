...
cr.plugins_.babylonNewMesh = function (runtime) {
...
(function () {
	...
	// *** CHANGE THE PLUGIN ID HERE *** - must match the "id" property in edittime.js
	//                            vvvvvvvv
	var pluginProto = cr.plugins_.babylonNewMesh.prototype;
	...
	instanceProto.tick2 = function () {
		...
		if (this.runtime.scenes[this.properties[1]]) { // Is scene defined
			if (!this.created && !this.runtime.scenes[this.properties[1]].getMeshByName("mesh" + this.uid)) // We didn't create the mesh yet
			{
				var rot = this.properties[4].split(","); //XY rotation
				...
				case 0:
					var mesh = BABYLON.Mesh.CreateBox("mesh" + this.uid, 1, scene, true);
					break; //Box
				case 1:
					var mesh = BABYLON.Mesh.CreateSphere("mesh" + this.uid, this.properties[7], 1, scene, true);
					break; //Sphere
				case 2:
					var mesh = BABYLON.Mesh.CreatePlane("mesh" + this.uid, 1, scene);
					break; //Plane
				case 3:
					var mesh = BABYLON.Mesh.CreateCylinder("mesh" + this.uid, 1, this.properties[8], this.properties[9], this.properties[10], 1, scene, true);
					break; //Cylinder
				case 4:
					var mesh = BABYLON.Mesh.CreateTorus("mesh" + this.uid, 1, this.properties[11], this.properties[10], scene, true);
					break; //Torus
				case 5:
					var mesh = BABYLON.Mesh.CreateGround("mesh" + this.uid, this.width, this.height, this.properties[7], scene, true);
					break; //Ground
				}
				mesh.position = new BABYLON.Vector3(xpos, ypos, zpos);
				mesh.rotation = new BABYLON.Vector3(rot[0] * (Math.PI / 180), rot[1] * (Math.PI / 180), -this.angle);
				
				if(this.properties[3] != 5)
				{
					mesh.scaling = new BABYLON.Vector3(this.width, this.height, this.properties[5]);
				}	
				//Materials//////////////////////////////////////////////
				var filenameDiffuse = getAnimTextureFileByName(this, "diffuse", this.properties[12]);
				var filenameBump = getAnimTextureFileByName(this, "bump", this.properties[12]);
				var filenameSpecular = getAnimTextureFileByName(this, "specular", this.properties[12]);
				var filenameEmissive = getAnimTextureFileByName(this, "emissive", this.properties[12]);
				var filenameAmbient = getAnimTextureFileByName(this, "ambient", this.properties[12]);
				var filenameHeight = getAnimTextureFileByName(this, "height", this.properties[12]);

				var mat = new BABYLON.StandardMaterial("mesh" + this.uid + "Mat", scene);
				if (filenameDiffuse != "null") {
					mat.diffuseTexture = new BABYLON.Texture(filenameDiffuse, scene);
					mat.diffuseTexture.uScale = this.properties[13]; //Repeat 5 times on the Vertical Axes
					mat.diffuseTexture.vScale = this.properties[14]; //Repeat 5 times on the Horizontal Axes
					mat.diffuseTexture.uOffset = this.properties[15];
					mat.diffuseTexture.vOffset = this.properties[16];
					mat.diffuseTexture.hasAlpha = this.properties[17];
					mat.backFaceCulling = this.properties[18]; //Always show the front and the back of an element
					mat.specularPower = this.properties[19];
					mat.roughness = this.properties[20];

				}
				if (filenameBump != "null") {
					mat.bumpTexture = new BABYLON.Texture(filenameBump, scene);
					mat.bumpTexture.uOffset = this.properties[15];
					mat.bumpTexture.vOffset = this.properties[16];
				}
				if (filenameSpecular != "null") {
					mat.specularTexture = new BABYLON.Texture(filenameSpecular, scene);
					mat.specularTexture.uOffset = this.properties[15];
					mat.specularTexture.vOffset = this.properties[16];
				}
				if (filenameEmissive != "null") {
					mat.emissiveTexture = new BABYLON.Texture(filenameEmissive, scene);
					mat.emissiveTexture.uOffset = this.properties[15];
					mat.emissiveTexture.vOffset = this.properties[16];
				}
				if (filenameAmbient != "null") {
					mat.ambientTexture = new BABYLON.Texture(filenameAmbient, scene);
					mat.ambientTexture.uOffset = this.properties[15];
					mat.ambientTexture.vOffset = this.properties[16];
				}
				if (filenameHeight != "null") {
					mesh.applyDisplacementMap(filenameHeight, this.properties[21], this.properties[22])
				}
				mesh.material = mat;
				...
				if (this.active_effect_types.length > 0) {
					...
					BABYLON.Effect.ShadersStore["custom" + this.uid + "VertexShader"] =
` 
...
void main(void) {
    ...
    vTex = uv;
					...
					BABYLON.Effect.ShadersStore["custom" + this.uid + "FragmentShader"] = cr.shaders[this.active_effect_types[0].id].src;
					/* var mat = new BABYLON.ShaderMaterial("matshader" + this.uid, scene,
					"custom", {
					attributes: ["position", "uv"],
					uniforms: ["worldViewProjection"]
					}); */

					this.shadermat = new BABYLON.ShaderMaterial("matshader" + this.uid, scene, {
							vertexElement: "custom" + this.uid,
							fragmentElement: "custom" + this.uid,
						}, {
							needAlphaBlending: false,
							attributes: ["position", "uv"],
							uniforms: ["worldViewProjection"],
							samplers: ["samplerFront"]
						});

					for (var i = 0; i < cr.shaders[this.active_effect_types[0].id].parameters.length; i++) {
						this.shadermat.setFloat(cr.shaders[this.active_effect_types[0].id].parameters[i][0], this.effect_params[0][i]);
					}
					this.shadermat.setFloat("pixelHeight", 0.01);
					this.shadermat.setFloat("pixelWidth", 0.01);
					this.shadermat.setTexture("samplerFront", mesh.material.diffuseTexture);
				...
				// getEffectIndexByName ; fx_count
				// effect_params ; active_effect_types ; active_effect_flags
				this.created = true;

				if (mesh.isReady) {
					this.runtime.trigger(cr.plugins_.babylonNewMesh.prototype.cnds.MeshOnReady, this);
				}
			}
	...
	instanceProto.tick = function () {
		if (this.shaderDone) {
			for (var i = 0; i < cr.shaders[this.active_effect_types[0].id].parameters.length; i++) {
				this.runtime.scenes[this.properties[1]].getMeshByName("mesh" + this.uid).material.setFloat(cr.shaders[this.active_effect_types[0].id].parameters[i][0], this.effect_params[0][i]);
			}
