[Plugin] Cordova PhoneCallStatus

1.Overview

check phone call status. (ringing, off hook, idle)

[android] [crodova cli] [xdk]

2.Change log

3.Install plugin

Now all the native plugins are installed automatically
https://plus.google.com/102658703990850475314/posts/XS5jjEApJYV

4.Server setting

not needed

5.API

//actions
Phone call status

//events
On phone call status ringing
On phone call status off hook
On phone call status idle
	
6.Examples

example capx are included in doc folder

7.Test

8.Useful links

Plugins For Cordova
http://cranberrygame.github.io?referrer=readme.txt
