...
//From shaderToy ;
// converted by Gigatron, ported by tatosgames
varying mediump vec2 vTex;
precision lowp float;

uniform float seconds;
uniform mediump float pixelWidth; //1.0/width or in other words a pixel width in vertex co-ordinates
uniform mediump float pixelHeight;
vec2 resolution = vec2( 1./pixelWidth, 1./pixelHeight);

uniform float pSpeed; //parametro esterno

float speed=seconds * pSpeed;
float hash(float n) { 
