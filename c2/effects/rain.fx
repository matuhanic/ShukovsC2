...
void main()
{
    ...
    float focus = mix(maxBlur-c.y, minBlur, S(.0, .0, c.x));
    vec3 col = texture2D(samplerFront, UV+n, focus).rgb;
    
    
 
   // t = (T+3.)*.5;										// make time sync with first lightnoing
    float colFade = sin(t*.2)*.5+.5;
    col *= mix(vec3(1.), vec3(.8, .9, 1.3), 0.0);	// subtle color shift
    float fade = 1.0;//S(0., 2., T);							// fade in at the start
    float lightning = sin(t*sin(t*10.));				// lighting flicker
    lightning *= pow(max(0., sin(t+sin(t))), 10.);		// lightning flash
    col *= 1.+lightning*fade*mix(1., .1, 1.0);	// composite lightning
