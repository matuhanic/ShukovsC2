...
uniform float seed;
uniform float waterpercentage;
uniform float grass_red;
uniform float grass_green;
uniform float grass_blue;
uniform float dirt_red;
uniform float dirt_green;
uniform float dirt_blue;
uniform float forest_red;
uniform float forest_green;
uniform float forest_blue;
uniform float desert_red;
uniform float desert_green;
uniform float desert_blue;
uniform float mountains_red;
uniform float mountains_green;
uniform float mountains_blue;
uniform float water_red;
uniform float water_green;
uniform float water_blue;
...
void main( void ) {
	...
	if (zz >= 0.0) {
		...
		float angle = mod(0.05 * seconds, TWOPI);
		...
		float w = waterpercentage/100.; // water percent
		...
		vec3 terrainColor = vec3(grass_red/255., grass_green/255., grass_blue/255.) * detail_l; //grass
		terrainColor = mix(terrainColor, vec3(dirt_red/255., dirt_green/255., dirt_blue/255.) * detail_l, smoothstep(b, b+0.3, 1.0-n3)); //bare dirt
		terrainColor = mix(terrainColor, vec3(forest_red/255., forest_green/255., forest_blue/255.) * detail_m, smoothstep(f, f+0.3, n2*tropical)); //forest
		terrainColor = mix(terrainColor, vec3(desert_red/255., desert_green/255., desert_blue/255.) * detail_l, smoothstep(d, d+0.3, n2*equatorial)); //desert
		terrainColor = mix(terrainColor, vec3(mountains_red/255., mountains_green/255., mountains_blue/255.) * detail_h, smoothstep(m, m+0.3, 1.0-n3)); //mountains
		
		color = mix(terrainColor, vec3(water_red/255.0, water_green/255.0, water_blue/255.) * detail_l, smoothstep(t, t+0.05, n)); //water
